...
def logger_level_set(str name, int lvl):
...
cdef class _Value:
    ...
    def select(self, str name, str selector=None):
...
cdef class _Type:
    ...
    def __init__(self, spec, str id=None, base=None):
...
cdef splitArgs(vector[string]& out, str s):
    for v in s.split(' '):
...
cdef class ClientOperation:
    ...
    def __init__(self, ClientProvider ctxt, str name,
...
cdef class ClientMonitor:
    ...
    def __init__(self, ClientProvider ctxt, str name, handler=None, _Value pvRequest=None):
        cdef string pvname = name.encode()
    ...
    def pop(self):
        if not self.sub.get():
            raise RuntimeError("Dead Monitor")
        return monPop(self.sub)
...
cdef class ClientProvider:
    def __init__(self, str provider, conf=None, useenv=True):
    ...
    def disconnect(self, str name=None):
        with nogil:
            self.ctxt.cacheClear()

    @staticmethod
    def makeRequest(str desc):
    ...
    def __init__(self, conf=None, useenv=True, providers=None):
        ...
        for prov in providers:
            ...
            if isinstance(prov, StaticProvider):
                sprov = prov
                with nogil:
                    self.serv.addSource(sprov.name, sprov.src.source())

            elif isinstance(prov, DynamicProvider):
                dprov = prov
                with nogil:
                    self.serv.addSource(dprov.name, dprov.src)

            elif isinstance(prov, Source):
                src = prov
                with nogil:
                    self.serv.addSource(src.name, src.src)
...
cdef class ServerOperation:
    ...
    def pvRequest(self):
        # TODO: need to pass through SharedPV/ExecOp
    ...
    def done(self, _Value value=None, str error=None):
        cdef string msg
...
cdef class DynamicProvider:
    ...
    def __init__(self, str name, handler):
        self.name = name.encode()
        self.handler = handler
